...
BOT_TOKEN = "8888202277:AAFCYBm4pnIOPTcWYh9hmKQu6_akty3_XtY"
ADMIN_ID = 0  # 👈 Замени эти цифры на свой реальный Telegram ID
...
ROOM_PRESETS = {
    ...
    8: [
        ["Адвокат", "Ниндзя", "Маньяк с бинтами", "Бессмертный", "Доктор", "Мирный житель", "Мирный житель",
         "Мирный житель"],
        ["Адвокат", "Ниндзя", "Маньяк с бинтами", "Бессмертный", "Тула", "Мирный житель", "Мирный житель",
         "Мирный житель"],
        ["Двуликий", "Ниндзя", "Маньяк с бинтами", "Бессмертный", "Доктор", "Мирный житель", "Мирный житель",
         "Мирный житель"],
    ...
    9: [
        ["Мафия", "Мафия", "Мафия", "Маньяк с бинтами", "Шериф", "Бессмертный", "Доктор", "Мирный житель",
         "Мирный житель"],
        ["Мафия", "Мафия", "Мафия", "Маньяк с бинтами", "Шериф", "Бессмертный", "Тула", "Мирный житель",
         "Мирный житель"],
        ["Адвокат", "Двуликий", "Ниндзя", "Маньяк с бинтами", "Доктор", "Бессмертный", "Шериф", "Мирный житель",
         "Мирный житель"],
        ["Адвокат", "Двуликий", "Ниндзя", "Маньяк с бинтами", "Тула", "Бессмертный", "Шериф", "Мирный житель",
         "Мирный житель"],
        ["Мафия", "Мафия", "Мафия", "Бессмертный", "Вор", "Маньяк с бинтами", "Мирный житель", "Мирный житель",
         "Мирный житель"],
    ...
    11: [
        ["Дон", "Ниндзя", "Адвокат", "Маньяк с бинтами", "Шериф", "Доктор", "Вор", "Бессмертный", "Мирный житель",
         "Мирный житель", "Мирный житель"]
    ],
    12: [
        ["Дон", "Ниндзя", "Адвокат", "Мафия", "Маньяк без бинтов", "Шериф", "Доктор", "Тула", "Вор", "Бессмертный",
         "Мирный житель", "Мирный житель"]
    ],
    13: [
        ["Дон", "Ниндзя", "Адвокат", "Мафия", "Маньяк с бинтами", "Шериф", "Доктор", "Тула", "Вор", "Бессмертный",
         "Двуликий", "Мирный житель", "Мирный житель"]
    ]
}
...
async def cmd_start_game(message: types.Message):
    ...
    if chat_id in games and games[chat_id].state not in ["FINISHED"]: return await message.answer(
        "Игра в этом чате уже запущена!")

    games[chat_id] = Game(chat_id)
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="✋ Присоединиться", callback_data="join_game")]])
...
async def join_game_handler(callback: types.CallbackQuery):
    ...
    if game.add_player(user.id, user.first_name):
        text = f"Зарегистрировано: {len(game.players)} чел.\n" + "\n".join(
            [f"{p.number}. {p.name}" for p in game.players.values()])
...
async def cmd_run(message: types.Message):
    ...
    if player_count not in ROOM_PRESETS: return await message.answer(
    ...
    for player in game.players.values():
        ...
        try:
            ...
            return await message.answer(
                f"Не удалось отправить роль игроку {player.name}. Он не нажал /start в личке с ботом!")
...
async def start_defense_phase(game: Game, chat_id: int):
    ...
    game.defense_queue = deque(
        [game.players_by_number[num] for num in game.nominated if game.players_by_number[num].is_alive])
...
async def cmd_nominate(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
                                                  [InlineKeyboardButton(text=f"№{t.number} ({t.name})",
                                                                        callback_data=f"nom|{message.chat.id}|{t.number}")]
                                                  for t in alive_players
                                              ] + [[InlineKeyboardButton(text="❌ Отмена (передумал)",
                                                                         callback_data=f"nom|{message.chat.id}|0")]])
...
async def cmd_nominated(message: types.Message):
    ...
    if game and game.nominated:
        await message.answer("Выставлены: " + ", ".join(map(str, game.nominated)))
    else:
        await message.answer("Пока никто не выставлен.")
...
async def next_defense_speaker(game: Game, chat_id: int):
    ...
    while game.defense_queue and game.defense_queue[0].is_glued:
        ...
        await bot.send_message(chat_id,
                               f"🤐 Игрок №{glued_p.number} заклеен Вором и пропускает свою оправдательную речь.")

    if game.defense_queue:
        next_p = game.defense_queue[0]
        await bot.send_message(chat_id,
                               f"🗣 Очередь оправдываться Игрока №{next_p.number}. Напишите /speech для начала речи.")
    else:
...
async def proceed_to_voting_or_autokick(game: Game, chat_id: int):
    if len(game.nominated) == 1:
        ...
        if game.players_by_number[killed_num].has_alibi:
            await bot.send_message(chat_id,
                                   f"🛡 Игрок №{killed_num} должен был покинуть стол, но у него оказалось АЛИБИ! Он выживает.")
        else:
...
async def cmd_vote(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"№{num} ({game.players_by_number[num].name})",
                              callback_data=f"v|{message.chat.id}|{num}")]
        for num in allowed
    ])
...
async def handle_vote_callback(callback: types.CallbackQuery):
    ...
    if not game.voting_queue:
        await calculate_votes(game, chat_id)
    else:
        await bot.send_message(chat_id, f"Следующий голосует Игрок №{game.voting_queue[0].number}. Напишите /vote")
...
async def handle_balance_callback(callback: types.CallbackQuery):
    ...
    if not game.voting_queue:
        await resolve_balance(game, chat_id)
    else:
        await bot.send_message(chat_id, f"Следующий голосует Игрок №{game.voting_queue[0].number}. Напишите /balance")
...
async def start_night_phase(game: Game, chat_id: int):
    ...
    if thief:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
                                                      [InlineKeyboardButton(text=f"№{t.number} ({t.name})",
                                                                            callback_data=f"n|{chat_id}|rek|{t.number}")]
                                                      for t in alive_players
                                                  ] + [[InlineKeyboardButton(text="Никого не клеить",
                                                                             callback_data=f"n|{chat_id}|rek|0")]])
    ...
    for p in alive_players:
        ...
        if actions:
            ...
            for act_code, text in actions:
                if act_code == "man_h":
                    kb = InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="Лечить себя", callback_data=f"n|{chat_id}|{act_code}|{p.number}")]])
                else:
                    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"№{t.number} ({t.name})",
                                                                                     callback_data=f"n|{chat_id}|{act_code}|{t.number}")]
                                                               for t in alive_players])
                try:
                    await bot.send_message(p.user_id, text, reply_markup=kb)
                except Exception as e:
                    print(f"Ошибка отправки игроку {p.name}: {e}")
...
async def handle_night_action(callback: types.CallbackQuery):
    ...
    game = games.get(chat_id)
    if not game or game.state not in ["NIGHT", "NIGHT_THIEF"]: return await callback.answer("Ночь уже прошла!",
                                                                                            show_alert=True)
    ...
    if act_code in ["heal", "tula"] and player.last_healed == target_num: return await callback.answer(
        "Нельзя лечить этого игрока две ночи подряд!", show_alert=True)
    if act_code == "alibi" and player.last_alibi == target_num: return await callback.answer(
        "Нельзя давать алиби этому игроку две ночи подряд!", show_alert=True)
    if act_code == "man_h" and getattr(player, 'last_man_heal', False): return await callback.answer(
        "Нельзя лечить себя 2 дня подряд!", show_alert=True)
    ...
    if act_code == "check_d":
        ...
        if t_player.role in game.mafia_team:
            ...
            for maf in game.get_alive_players():
                if maf.role in game.mafia_team: await bot.send_message(maf.user_id,
