...
class TestRotatingLogger:
    ...
    def logger(self, temp_log_file):
        return setup_rotating_logger(
            temp_log_file, max_bytes=10000, backup_count=3, logger_name="test_logger"
        )
    ...
    def test_rotation_by_size(self, temp_log_file):
        max_bytes = 500
        backup_count = 2
        small_logger = setup_rotating_logger(
            temp_log_file, max_bytes, backup_count, logger_name="test_logger"
        )
        ...
        for i in range(messages_count):
            small_logger.info(
                f"Message number {i} with some extra text to make it longer"
            )
    ...
    def test_backup_count_limit(self, temp_log_file):
        ...
        assert (
            len(log_files) <= max_expected_files
    ...
    def test_logger_configuration(self, temp_log_file):
        max_bytes = 2048
        backup_count = 5
        test_logger = setup_rotating_logger(
            temp_log_file, max_bytes, backup_count, logger_name="config_logger"
        )
    ...
    def test_rotation_does_not_lose_messages(self, temp_log_file):
        max_bytes = 2500
        small_logger = setup_rotating_logger(
            temp_log_file, max_bytes, backup_count=2, logger_name="rotation_logger"
        )
...
class TestLoggerPerformance:
    def test_write_performance(self, tmp_path):
        ...
        logger = setup_rotating_logger(
            str(log_file), max_bytes=1000000, backup_count=3, logger_name="test_logger"
        )
