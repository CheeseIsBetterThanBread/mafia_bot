...
class TestEventDispatcher:
    ...
    async def test_route_to_correct_handler(self, dispatcher, cmd_value, handler_name):
        ...
        with patch.object(
            dispatcher, handler_name, new_callable=AsyncMock
        ) as mock_handler:
            await dispatcher.handle(query)
    ...
    async def test_send_response_base(self, dispatcher):
        with patch.object(
            dispatcher, "_EventDispatcher__send_response", new_callable=AsyncMock
        ) as mock_send:
            await dispatcher._EventDispatcher__send_response_base(
                123, "Test", "HTML", True
            )
    ...
    async def test_send_response_with_options(self, dispatcher):
        ...
        with patch.object(
            ...
            await dispatcher._EventDispatcher__send_response_with_options(
                candidates, 123, "Test", "HTML", True
            )
